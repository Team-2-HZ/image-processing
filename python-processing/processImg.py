...
def process_food_from_image():
    ...
    # its predicting time , our model will try to predict the prob of the particular class
    pred = loaded_model.predict(random_images)
    pred = np.argmax(pred, axis=1)
    # predicted_label = list(labels_food.keys())[pred[0]]
    # return the predicted labels as a list
    predicted_labels = [list(labels_food.keys())[pred[i]]
